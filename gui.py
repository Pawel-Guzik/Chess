...
class Gui(tk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.registerButton = tk.Button(self, text="REGISTER", command=self.register, pady=5, bg=self.white,
                                        font='arial')
        self.createButton = tk.Button(self, text="CREATE AN ACCOUNT", command=self.createAccount, pady=5, bg=self.white,
                                      font='arial')
        ...
        self.gameHistoryButton = tk.Button(self, text="GAME HISTORY", command=self.gameHistory, pady=5, bg=self.white,
                                           font='arial')
    ...
    def login(self):
        ...
        mycursor.execute("SELECT nickname, password, points FROM user WHERE nickname = %s and password = %s",
                         (nick, passw))
    ...
    def menu(self):
        self.resetGrid()
        tk.Label(self, text=self.userInfo, pady=4, bg=self.black, font=('arial', 25), fg=self.white).grid(row=0,
                                                                                                          column=0)
    ...
    def gameHistory(self):
        ...
        self.resetGrid()
        mycursor = db.cursor()
        mycursor.execute("SELECT * FROM game_history WHERE P1_nick=%s or P2_nick=%s",
                         (self.loggedUser, self.loggedUser))
        allGames = mycursor.fetchall()
        tk.Label(self, text="GAME HISTORY", pady=4, bg=self.black, font=('arial', 25), fg=self.white).grid(row=0,
                                                                                                           column=0)
        for row_number, game in enumerate(allGames):
            ...
            if game[3] == self.loggedUser:
                ...
                color = 'red'
            tk.Label(self, text=ltxt, pady=4, bg=self.black, font=('arial', 15), fg=color).grid(row=row_number + 1,
                                                                                                column=0)
    ...
    def ranking(self):
        ...
        tk.Label(self, text="TOP 10 PLAYERS", pady=4, bg=self.black, font=('arial', 25), fg=self.white).grid(row=0,
                                                                                                             column=0)
        for pos, player in enumerate(allUsers):
            if pos == 10:
                break
            txt = f'{(pos + 1):2} {player[0]:8} {player[1]:5}'
            tk.Label(self, text=txt, pady=4, bg=self.black, font=('arial', 15), fg=self.white).grid(row=pos + 1,
                                                                                                    column=0)
        self.menuButton.grid(row=12, column=0)
